...
from bs4.element import SoupStrainer, XMLProcessingInstruction
...
def cath_referencia(data_ultimo_dado):
  ...
  print("Ultima informação: " + str(data_ultimo_dado))
  print("Mês: " + str(mes))
...
def cath_serie():
  ...
  for mes in lista_periodos:
    ...
    soup = bs(dados.content, "html.parser")
...
def define_informacoes_cartao(series, serie_escolhida):
  valor_periodo_anterior = str(series[serie_escolhida][-2:][0]['y'])
  valor_periodo_atual = str(series[serie_escolhida][-2:][1]['y'])
