...
def cath_dados(num_api, num_url, id_serie_target): 
  ...
  print(importacoes_referencia)
  print('- Dados de extraídos')
  ################### referencia
  # Formarto Jan-Mai/2020
  ...
  #Formarto Mai 2020 - Mai 2021 - solicitacao de alteracao feita em 07/06
  importacoes_referencia = importacoes_referencia.split('-')[-1]
  importacoes_referencia = importacoes_referencia.split('/')
  importacoes_referencia = importacoes_referencia[0]+' '+ str(int(importacoes_referencia[1])-1) + ' - ' + importacoes_referencia[0]+' '+importacoes_referencia[1]
  
  exportacoes_referencia = exportacoes_referencia.split('-')[1]             
  exportacoes_referencia = exportacoes_referencia.split('/')
  exportacoes_referencia = exportacoes_referencia[0]+' '+ str(int(exportacoes_referencia[1])-1) + ' - ' + exportacoes_referencia[0]+' '+exportacoes_referencia[1]
    ...
    'stats': [
        {
            ...
            'cor_seta': 'orange',
            'desc_serie': 'Variação percentual em relação ao mesmo mês do ano anterior e Valor FOB por mês',
            'serie_tipo': 'data',
